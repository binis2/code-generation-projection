...
public class MapBackedProjectionProvider {
    ...
    public static <T> T create(Class<T> projection, Map<String, Object> map) {
        try {
            var proxyClass = getOrCreateProxyClass(projection);
            var instance = CodeFactory.create(proxyClass, map);
            return (T) instance;
        } catch (Exception e) {
            throw new RuntimeException("Failed to create projection instance for " + projection.getName(), e);
        }
    }

    private static Class<?> getOrCreateProxyClass(Class<?> projection) {
        return CLASS_CACHE.computeIfAbsent(projection, cls -> {
            try {
                return BYTE_BUDDY
                        ...
                        .make()
                        .load(projection.getClassLoader())
                        .getLoaded();
            } catch (Exception e) {
                throw new RuntimeException("Failed to create proxy class for " + projection.getName(), e);
            }
        });
    }
